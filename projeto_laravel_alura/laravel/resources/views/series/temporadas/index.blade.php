...
@section('conteudo')
    ...
    <div class="card ">
        <div class="card-body">
            <a href="{{ route('listar_series') }}" class="btn btn-dark mb-4" name="" id="" role="button">
                Voltar
            </a>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    <span class="badge badge-secondary">
                                        {{ $tp->episodios->where('status', '=', true)->count() }} /
                                        {{ $tp->episodios->count() }}
                                    </span>
                                </td>
                                <td class="align-middle">
                                    <form method="post"
                                        action="{{ route('listar_episodios', ['id_serie' => $serie->id, 'id_temporada' => $tp->id]) }}">
                                        @csrf
                                        @method('get')
